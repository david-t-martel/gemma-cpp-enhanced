...
// Windows-optimized version for gemma.cpp with enhanced MSVC compatibility.

#ifndef HIGHWAY_SCALAR_FALLBACK_WINDOWS_H_
#define HIGHWAY_SCALAR_FALLBACK_WINDOWS_H_
...
// ------------------------------ PromoteLowerTo// Promote lower half to wider typetemplate <class D, class V, typename T = TFromD<D>>HWY_SCALAR_INLINE VFromD<D> PromoteLowerTo(D d, V v) {  // For scalar: just convert the single element to the target type (same as upper)  using TFrom = TFromV<V>;  static_assert(sizeof(T) == 2 * sizeof(TFrom), "Target should be twice the width");  return Set(d, static_cast<T>(GetLane(v)));}
// ------------------------------ PromoteOddTo
...
#endif  // HIGHWAY_SCALAR_FALLBACK_WINDOWS_H_