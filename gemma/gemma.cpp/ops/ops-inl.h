...
// Min-P sampling: filters tokens based on their probability relative to the most likely token
template <typename TAcceptToken>
HWY_NOINLINE HWY_MAYBE_UNUSED std::vector<TokenAndProb> MinPFilter(
    const float* HWY_RESTRICT probabilities, size_t vocab_size, float min_p,
    TAcceptToken& accept_token) {
  HWY_ASSERT(min_p >= 0.0f && min_p <= 1.0f);

  if (min_p <= 0.0f) {
    // No filtering when min_p is 0 or negative
    std::vector<TokenAndProb> all_tokens;
    all_tokens.reserve(vocab_size);
    for (size_t i = 0; i < vocab_size; ++i) {
      if (!accept_token || accept_token(static_cast<int>(i), probabilities[i])) {
        all_tokens.push_back(TokenAndProb{.token = static_cast<int>(i), .prob = probabilities[i]});
      }
    }
    return all_tokens;
  }
  ...
  // Calculate threshold
  const float threshold = min_p * max_prob;

  std::vector<TokenAndProb> filtered_tokens;
  for (size_t i = 0; i < vocab_size; ++i) {
    if (probabilities[i] >= threshold &&
        (!accept_token || accept_token(static_cast<int>(i), probabilities[i]))) {
      filtered_tokens.push_back(TokenAndProb{.token = static_cast<int>(i), .prob = probabilities[i]});
    }
  }
...
// Typical sampling: filters tokens based on their typical probability
template <typename TAcceptToken>
HWY_NOINLINE HWY_MAYBE_UNUSED std::vector<TokenAndProb> TypicalPFilter(
    const float* HWY_RESTRICT probabilities, size_t vocab_size, float typical_p,
    TAcceptToken& accept_token) {
  HWY_ASSERT(typical_p >= 0.0f && typical_p <= 1.0f);

  if (typical_p >= 1.0f) {
    // No filtering when typical_p is 1.0 or greater
    std::vector<TokenAndProb> all_tokens;
    all_tokens.reserve(vocab_size);
    for (size_t i = 0; i < vocab_size; ++i) {
      if (!accept_token || accept_token(static_cast<int>(i), probabilities[i])) {
  ...
  // Calculate entropy and expected value
  double entropy = 0.0;
  for (size_t i = 0; i < vocab_size; ++i) {
  ...
  // Calculate information content for each token and sort by deviation from entropy
  std::vector<std::pair<double, size_t>> token_info;
  for (size_t i = 0; i < vocab_size; ++i) {
    if (probabilities[i] > 0.0f &&
        (!accept_token || accept_token(static_cast<int>(i), probabilities[i]))) {
      double info_content = -std::log(probabilities[i]);
      double deviation = std::abs(info_content - entropy);
      token_info.push_back(std::make_pair(deviation, i));
    }
  }

  // Sort by deviation (ascending - tokens closest to typical information content first)
  std::sort(token_info.begin(), token_info.end());

  // Select tokens up to typical_p cumulative probability
  std::vector<TokenAndProb> typical_tokens;
  double cumulative_prob = 0.0;
  for (const auto& pair : token_info) {
    size_t token_idx = pair.second;
    typical_tokens.push_back(TokenAndProb{.token = static_cast<int>(token_idx),
                                          .prob = probabilities[token_idx]});
    cumulative_prob += probabilities[token_idx];
    if (cumulative_prob >= typical_p) {
      break;
    }
  }

  return typical_tokens;
}
...
// MOVED TO SEPARATE FILE: ops/dry_penalty.h to avoid multiple definition errors
/*
HWY_INLINE HWY_MAYBE_UNUSED void ApplyDRYPenalty(
  ...
  // For each potential next token, check if it would extend a repetitive sequence
  for (size_t token_idx = 0; token_idx < vocab_size; ++token_idx) {
    const int candidate_token = static_cast<int>(token_idx);

    // Look for matching sequences in recent history
    size_t max_repeat_length = 0;

    // Check all possible starting positions for repeated sequences
    for (size_t start_pos = 0; start_pos < num_recent; ++start_pos) {
      if (recent_tokens[recent_tokens.size() - 1 - start_pos] != candidate_token) {
        continue;  // This position doesn't match the candidate token
      }

      // Found a match, now check how long the repetition would be
      size_t repeat_length = 1;
      size_t check_pos = start_pos + 1;

      while (check_pos < num_recent && repeat_length <= dry_allowed_length + 10) {
        // Check if the pattern repeats
        size_t pattern_pos = repeat_length % (start_pos + 1);
        if (recent_tokens[recent_tokens.size() - 1 - check_pos] !=
            recent_tokens[recent_tokens.size() - 1 - pattern_pos]) {
          break;
        }
        repeat_length++;
        check_pos++;
      ...
      max_repeat_length = HWY_MAX(max_repeat_length, repeat_length);
    }

    // Apply penalty if repetition exceeds allowed length
    if (max_repeat_length > dry_allowed_length) {
      const float penalty_exp = static_cast<float>(max_repeat_length - dry_allowed_length);
      const float penalty = dry_multiplier * std::pow(dry_base, penalty_exp);
...
// Dynamic temperature implementation: adjusts temperature based on entropy
HWY_INLINE float CalculateDynamicTemperature(const float* HWY_RESTRICT probabilities,
                                             size_t vocab_size, float base_temperature,
                                             float dynatemp_range, float dynatemp_exponent) {
  ...
  const double max_entropy = std::log(vocab_size);
  const double normalized_entropy = entropy / max_entropy;

  // Apply exponent for non-linear mapping
  const double adjusted_entropy = std::pow(normalized_entropy, dynatemp_exponent);

  // Map to temperature range: low entropy -> high temperature, high entropy -> low temperature
  const float temperature_adjustment = dynatemp_range * (1.0f - static_cast<float>(adjusted_entropy));

  return base_temperature + temperature_adjustment;
}
...
// Advanced sampling with multiple filtering methods
template <typename TAcceptToken>
HWY_NOINLINE HWY_MAYBE_UNUSED TokenAndProb AdvancedSample(
    float* HWY_RESTRICT logits, size_t vocab_size, std::mt19937& gen,
    float temperature, float min_p, float typical_p, size_t top_k,
    float dynatemp_range, float dynatemp_exponent,
    const hwy::Span<const int>& recent_tokens, float dry_multiplier,
    float dry_base, size_t dry_allowed_length, size_t dry_penalty_last_n,
    TAcceptToken& accept_token, hwy::Profiler& p, size_t worker) {

  // Apply DRY penalty to logits first (before softmax)
  if (dry_multiplier > 0.0f && recent_tokens.size() > 0) {
    ApplyDRYPenalty(logits, vocab_size, recent_tokens, dry_multiplier,
                    dry_base, dry_allowed_length, dry_penalty_last_n);
  }

  // Apply softmax to get probabilities
  Softmax(logits, vocab_size, p, worker, 1.0f);  // Use temperature=1.0 for initial softmax

  // Calculate dynamic temperature if enabled
  float effective_temperature = temperature;
  if (dynatemp_range > 0.0f) {
    effective_temperature = CalculateDynamicTemperature(
        logits, vocab_size, temperature, dynatemp_range, dynatemp_exponent);
  }

  // Apply temperature if different from 1.0
  if (effective_temperature != 1.0f) {
    const float temp_inv = 1.0f / effective_temperature;
    namespace hn = hwy::HWY_NAMESPACE;
    using D = hn::ScalableTag<float>;
    hn::Transform(D(), logits, vocab_size,
                  [temp_inv](D d, hn::Vec<D> v) HWY_ATTR {
                    return hn::Exp(d, hn::Mul(hn::Log(d, v), hn::Set(d, temp_inv)));
                  });

    // Re-normalize after temperature scaling
    const float sum = Sum(hn::ScalableTag<float>(), logits, vocab_size);
    const float norm = 1.0f / sum;
    MulByConst(norm, logits, vocab_size, p, worker);
  }

  std::vector<TokenAndProb> candidate_tokens;

  // Apply filtering in order: Min-P, Typical-P, Top-K
  if (min_p > 0.0f && min_p < 1.0f) {
    candidate_tokens = MinPFilter(logits, vocab_size, min_p, accept_token);
  } else if (typical_p > 0.0f && typical_p < 1.0f) {
    candidate_tokens = TypicalPFilter(logits, vocab_size, typical_p, accept_token);
  } else if (top_k > 1 && top_k < vocab_size) {
    candidate_tokens = TopK(logits, vocab_size, top_k, accept_token);
  } else {
    // No filtering, use all tokens
    candidate_tokens.reserve(vocab_size);
    for (size_t i = 0; i < vocab_size; ++i) {
      if (!accept_token || accept_token(static_cast<int>(i), logits[i])) {
        candidate_tokens.push_back(TokenAndProb{.token = static_cast<int>(i), .prob = logits[i]});
      }
    }
  }

  if (candidate_tokens.empty()) {
    // Fallback to most likely token if no candidates remain
    return ArgmaxAndMax(logits, vocab_size);
  }

  // Sample from filtered candidates
  std::vector<float> candidate_probs;
  candidate_probs.reserve(candidate_tokens.size());
  for (const auto& token_prob : candidate_tokens) {
    candidate_probs.push_back(token_prob.prob);
  }
  ...
  return candidate_tokens[selected_idx];
}
