...
ChooseSampleFunc(const RuntimeConfig& runtime_config, ThreadingContext& ctx) {
  ...
  // Check if advanced sampling is enabled
  const bool use_advanced_sampling =
      runtime_config.min_p > 0.0f ||
      runtime_config.typical_p < 1.0f ||
      runtime_config.dynatemp_range > 0.0f ||
      runtime_config.dry_multiplier > 0.0f;

  // Fast path for top-1 with no accept_token and no advanced sampling.
  if (runtime_config.top_k == 1 && !runtime_config.accept_token && !use_advanced_sampling) {
  ...
  // Advanced sampling case
  if (use_advanced_sampling) {
    // For DRY penalty, we need to track recent tokens.
    // Create a shared token history storage (simplified per-session approach).
    static thread_local std::vector<int> recent_tokens;

    return [&](float* logits, size_t vocab_size) HWY_ATTR -> TokenAndProb {
      PROFILER_ZONE("Gen.Sample advanced");

      // Convert to hwy::Span for the function call
      hwy::Span<const int> recent_span(recent_tokens.data(), recent_tokens.size());

      TokenAndProb result = AdvancedSample(
          logits, vocab_size, *runtime_config.gen,
          runtime_config.temperature, runtime_config.min_p,
          runtime_config.typical_p, runtime_config.top_k,
          runtime_config.dynatemp_range, runtime_config.dynatemp_exponent,
          recent_span, runtime_config.dry_multiplier,
          runtime_config.dry_base, runtime_config.dry_allowed_length,
          runtime_config.dry_penalty_last_n,
          runtime_config.accept_token, ctx.profiler, worker);

      // Update token history for DRY penalty
      if (runtime_config.dry_multiplier > 0.0f) {
        recent_tokens.push_back(result.token);
        // Keep only the last N tokens to avoid unbounded growth
        if (recent_tokens.size() > runtime_config.dry_penalty_last_n) {
          recent_tokens.erase(recent_tokens.begin(),
                              recent_tokens.end() - runtime_config.dry_penalty_last_n);
        }
      }
  ...
  // Legacy case: Standard top-k sampling.
  return [&](float* logits, size_t vocab_size) HWY_ATTR -> TokenAndProb {
    PROFILER_ZONE("Gen.Sample topk");
