...
struct RuntimeConfig {
  ...
  float min_p = 0.0f;         // Min-P sampling threshold (0.0 = disabled).
  float typical_p = 1.0f;     // Typical-P sampling threshold (1.0 = disabled).

  // Dynamic temperature parameters
  float dynatemp_range = 0.0f;    // Dynamic temperature range (0.0 = disabled).
  float dynatemp_exponent = 1.0f; // Dynamic temperature exponent.

  // DRY (Don't Repeat Yourself) penalty parameters
  float dry_multiplier = 0.0f;     // DRY penalty multiplier (0.0 = disabled).
  float dry_base = 1.75f;          // DRY penalty base.
  size_t dry_allowed_length = 2;   // Allowed repetition length before penalty.
  size_t dry_penalty_last_n = 256; // Number of recent tokens to consider for DRY penalty.
...
struct InferenceArgs : public ArgsBase<InferenceArgs> {
  ...
  void ForEach(const Visitor& visitor) {
    ...
    visitor(min_p, "min_p", 0.0f,
            "Min-P sampling: minimum probability threshold relative to most likely token (0.0 = disabled)", 2);
    visitor(typical_p, "typical_p", 1.0f,
            "Typical-P sampling: probability threshold for typical tokens (1.0 = disabled)", 2);

    visitor(dynatemp_range, "dynatemp_range", 0.0f,
            "Dynamic temperature range: temperature adjustment based on entropy (0.0 = disabled)", 2);
    visitor(dynatemp_exponent, "dynatemp_exponent", 1.0f,
            "Dynamic temperature exponent: controls non-linearity of entropy mapping", 2);

    visitor(dry_multiplier, "dry_multiplier", 0.0f,
            "DRY penalty multiplier: strength of repetition penalty (0.0 = disabled)", 2);
    visitor(dry_base, "dry_base", 1.75f,
            "DRY penalty base: exponential base for repetition penalty", 2);
    visitor(dry_allowed_length, "dry_allowed_length", size_t{2},
            "DRY allowed length: maximum repetition length before penalty", 2);
    visitor(dry_penalty_last_n, "dry_penalty_last_n", size_t{256},
            "DRY penalty last N: number of recent tokens to consider for repetition", 2);
  ...
  void CopyTo(RuntimeConfig& runtime_config) const {
    ...
    runtime_config.min_p = min_p;
    runtime_config.typical_p = typical_p;
    runtime_config.dynatemp_range = dynatemp_range;
    runtime_config.dynatemp_exponent = dynatemp_exponent;
    runtime_config.dry_multiplier = dry_multiplier;
    runtime_config.dry_base = dry_base;
    runtime_config.dry_allowed_length = dry_allowed_length;
    runtime_config.dry_penalty_last_n = dry_penalty_last_n;
  }
};
