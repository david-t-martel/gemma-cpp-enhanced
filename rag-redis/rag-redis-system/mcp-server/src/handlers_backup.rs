...
impl McpHandler {
    ...
    /// Warm up the RAG system components for better first-request performance
    pub async fn warm_up(&self) -> RagResult<()> {
        info!("Warming up MCP handler components");
        self.rag_system.warm_up().await.map_err(|e| {
            error!("Warm-up failed: {}", e);
            e
        })?;
        info!("MCP handler warm-up completed");
        Ok(())
    }
